...
import {
  actionTypes,
  getPostDetailBySlugSuccess,
  getPostsByKeyWordSuccess,
} from "./action";
...
function* getPostItemsByKeywordSaga({ payload }) {
  try {
    const data = yield call(PostRepository.getPostItemsByKeyword, payload);
...
function* getPostItemsByCategory({ payload }) {
  try {
    const data = yield call(PostRepository.getPostItemsByCategory, payload);
    // console.log(data);
    yield put(getPostsByKeyWordSuccess(data));
  } catch (err) {
    console.log(err);
